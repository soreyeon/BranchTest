...
<body>
	...
	<caption><h2>자료실</h2></caption>
		<tr>
			...
			<td style="width:350px; height:30px; text-align:center">제목</td>
			...
			<tr>
				...
				<c:choose>
					...
					<c:when test="${ pdsVo.lvl eq 0 }">
						<c:choose>
							<c:when test="${ pdsVo.delnum eq 0 }">							
								<a href="/Pds/View?idx=${ pdsVo.idx }&menu_id=${ menu_id }&nowpage=${ nowpage }&pagegrpnum=${ pagegrpnum }">
									${ pdsVo.title }
								</a>				
							</c:when>
							<c:otherwise>
					...
					<c:otherwise>
						<c:choose>
							<c:when test="${ pdsVo.delnum eq 0 }">
								<a href="/Pds/View?idx=${ pdsVo.idx }&menu_id=${ menu_id }">[Re]: ${ pdsVo.title }</a>
							</c:when>
							<c:otherwise>
								[RE]: <s>게시물이 삭제되었습니다.</s>
							</c:otherwise>
		...
		<tr>
			<td style="height:30px; text-align:right" colspan="6">
				<a href="/Pds/WriteForm?menu_id=${ menu_id }&bnum=0&lvl=0&step=0&nref=0&nowpage=${ nowpage }&pagegrpnum=${ pagegrpnum }">글쓰기</a>
				<a href="/">메인으로</a>
			</td>
		</tr>
